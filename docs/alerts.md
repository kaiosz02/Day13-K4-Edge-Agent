...
- **Ba bước kiểm tra đầu tiên:**
  1. Kiểm tra `/metrics` → xem `latency_p95` và `latency_p99` thực tế
...
- **Ba bước kiểm tra đầu tiên:**
  1. Kiểm tra `/metrics` → xem `error_breakdown` để biết loại lỗi phổ biến nhất
...
- **Ba bước kiểm tra đầu tiên:**
  1. Kiểm tra `/metrics` → xem `quality_avg` hiện tại
